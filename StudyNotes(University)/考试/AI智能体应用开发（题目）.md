...
1. Python 语言的英国发音是 `` `[填空]` ``，其语言拥有者是 `` `[填空]` `` 基金会。
2. Python 缩进规则中，1 个缩进等于 `` `[填空]` `` 个空格；单行注释使用 `` `[填空]` `` 符号。
3. Python 中对列表进行反转应使用 `` `[填空]` `` 方法；for 循环遍历数字序列应使用 `` `[填空]` `` 函数。
4. 列表推导式使用 `` `[填空]` `` 定义，而生成器表达式使用 `` `[填空]` `` 定义，后者返回一个生成器对象，实现惰性求值。
5. FastAPI 自动生成的 API 文档包括 Swagger UI 和 ReDoc，分别通过 `` `[填空]` `` 和 `` `[填空]` `` 路径访问。
6. FastAPI 的依赖注入系统通过 `` `[填空]` `` 装饰器实现；定义请求体参数时通常使用 Pydantic 库的 `` `[填空]` `` 类。
7. SQLModel 的核心库除了 SQLAlchemy 之外，还包括 `` `[填空]` ``、asyncpg 和 aiosqlite。
